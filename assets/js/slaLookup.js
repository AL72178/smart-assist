...
(function () {
  ...
  async function fetchSLAData() {
    if (window.newSLAData) {
      ...
      throw new Error('SLA data is unavailable locally.');
    }
  ...
  function renderClaimResults(categoryData, categoryLabel, stateNotes) {
    ...
    if (categoryData.nonPar) {
      slaResults.appendChild(
        createClaimCard(categoryLabel, 'Non-PAR', 'Professional', categoryData.nonPar.professional),
      );
